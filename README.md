<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0d0d,50:00ff41,100:0d0d0d&height=220&section=header&text=Chinthan%20Rai&fontSize=60&fontColor=00ff41&animation=fadeIn&fontAlignY=38&desc=Initializing%20System...&descSize=20&descAlignY=60&descColor=00ff41" width="100%"/>

</div>

<div align="center">

```
 ██████╗██╗  ██╗██╗███╗   ██╗████████╗██╗  ██╗ █████╗ ███╗   ██╗    ██████╗  █████╗ ██╗
██╔════╝██║  ██║██║████╗  ██║╚══██╔══╝██║  ██║██╔══██╗████╗  ██║    ██╔══██╗██╔══██╗██║
██║     ███████║██║██╔██╗ ██║   ██║   ███████║███████║██╔██╗ ██║    ██████╔╝███████║██║
██║     ██╔══██║██║██║╚██╗██║   ██║   ██╔══██║██╔══██║██║╚██╗██║    ██╔══██╗██╔══██║██║
╚██████╗██║  ██║██║██║ ╚████║   ██║   ██║  ██║██║  ██║██║ ╚████║    ██║  ██║██║  ██║██║
 ╚═════╝╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝    ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝
```

</div>

<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Share+Tech+Mono&size=22&duration=2500&pause=800&color=00FF41&center=true&vCenter=true&width=750&height=60&lines=%3E+Full+Stack+Dev+%7C+DevOps+%7C+QA+Engineer;%3E+MERN+%7C+Docker+%7C+AWS+EC2+%7C+CI%2FCD;%3E+Selenium+WebDriver+%7C+Playwright+%7C+POM;%3E+55%2B+REST+Endpoints+Validated+%E2%9C%85;%3E+DevOps+Intern+%40+Rooman+%E2%80%94+100%2F100+%F0%9F%94%A5;%3E+SYSTEM+READY." alt="Typing SVG" />
</div>

<br/>

<div align="center">

![Status](https://img.shields.io/badge/STATUS-ONLINE-00ff41?style=for-the-badge&logo=statuspage&logoColor=black&labelColor=0d0d0d)
![Location](https://img.shields.io/badge/LOCATION-MANGALURU,_INDIA-00ff41?style=for-the-badge&logo=googlemaps&logoColor=black&labelColor=0d0d0d)
![Role](https://img.shields.io/badge/OPEN_TO-FULLSTACK_%7C_DEVOPS_%7C_QA-00ff41?style=for-the-badge&logo=handshake&logoColor=black&labelColor=0d0d0d)

</div>

---

## `> whoami`

<table>
<tr>
<td width="55%" valign="top">

```typescript
interface Developer {
  name:      "Chinthan Rai Kukkuvalli";
  alias:     "ChinthanRai";
  roles:     ["Full-Stack Dev", "DevOps Engineer", "Manual & Automated Testing", "QA Engineer"];
  location:  "Mangaluru, Karnataka, India";
  education: "MCA @ Srinivas Institute of Technology (VTU)";
  cgpa:      "8.57";

  experience: {
    title:  "DevOps Intern @ Rooman Technologies";
    period: "Feb 2026 – May 2026";
    score:  "100 / 100 🔥";
    work:   [
      "AWS EC2 · IAM · VPC · S3",
      "Docker & Docker Compose",
      "GitHub Actions CI/CD Pipelines",
      "Bash Automation Scripts",
      "Smoke & Sanity Checks Post-Deploy"
    ];
  };

  stack: {
    devops:    ["Docker", "GitHub Actions", "AWS EC2/S3/IAM/VPC", "Nginx"];
    qa:        ["Selenium WebDriver (Java)", "Playwright (JS/TS)", "POM"];
    testing:   ["Functional", "Regression", "Smoke", "Sanity", "API Validation"];
    frontend:  ["React.js", "JavaScript ES6+", "HTML5", "CSS3"];
    backend:   ["Node.js", "Express.js", "JWT Auth", "OTP Workflows", "MVC"];
    databases: ["MongoDB (Mongoose)", "MySQL"];
    languages: ["Python", "JavaScript", "Java", "C", "C++", "Bash"];
  };

  currentlyLearning: ["Kubernetes", "Terraform", "TypeScript", "Advanced AWS"];
  philosophy: "Automate everything. Ship fast. Break nothing.";
  status:     "Open to Full-Stack / DevOps / Testing & QA roles 🟢";
}
```

</td>
<td width="45%" valign="top" align="center">

<img src="https://github-readme-stats.vercel.app/api?username=ChinthanRai&show_icons=true&theme=chartreuse-dark&hide_border=true&bg_color=0d0d0d&title_color=00ff41&icon_color=00ff41&text_color=ffffff" width="100%"/>

<br/>

<img src="https://streak-stats.demolab.com?user=ChinthanRai&theme=dark&hide_border=true&background=0d0d0d&ring=00ff41&fire=00ff41&currStreakLabel=00ff41&sideLabels=00ff41&dates=ffffff&stroke=00ff41" width="100%"/>

</td>
</tr>
</table>

---

## `> cat skills.json`

### ⚡ DevOps & Cloud

![Docker](https://img.shields.io/badge/Docker-0d0d0d?style=for-the-badge&logo=docker&logoColor=00ff41)
![Docker Compose](https://img.shields.io/badge/Docker_Compose-0d0d0d?style=for-the-badge&logo=docker&logoColor=00ff41)
![AWS EC2](https://img.shields.io/badge/AWS_EC2-0d0d0d?style=for-the-badge&logo=amazon-aws&logoColor=00ff41)
![AWS S3](https://img.shields.io/badge/AWS_S3-0d0d0d?style=for-the-badge&logo=amazons3&logoColor=00ff41)
![AWS IAM](https://img.shields.io/badge/AWS_IAM-0d0d0d?style=for-the-badge&logo=amazon-aws&logoColor=00ff41)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-0d0d0d?style=for-the-badge&logo=github-actions&logoColor=00ff41)
![Linux](https://img.shields.io/badge/Linux_CLI-0d0d0d?style=for-the-badge&logo=linux&logoColor=00ff41)
![Nginx](https://img.shields.io/badge/Nginx-0d0d0d?style=for-the-badge&logo=nginx&logoColor=00ff41)
![Kubernetes](https://img.shields.io/badge/Kubernetes_(Fundamentals)-0d0d0d?style=for-the-badge&logo=kubernetes&logoColor=00ff41)

### 🧪 QA & Test Automation

![Selenium](https://img.shields.io/badge/Selenium_WebDriver_(Java)-0d0d0d?style=for-the-badge&logo=selenium&logoColor=00ff41)
![Playwright](https://img.shields.io/badge/Playwright_(JS%2FTS)-0d0d0d?style=for-the-badge&logo=playwright&logoColor=00ff41)
![POM](https://img.shields.io/badge/Page_Object_Model-0d0d0d?style=for-the-badge&logoColor=00ff41)
![Manual Testing](https://img.shields.io/badge/Manual_Testing-0d0d0d?style=for-the-badge&logoColor=00ff41)
![API Testing](https://img.shields.io/badge/REST_API_Validation-0d0d0d?style=for-the-badge&logo=postman&logoColor=00ff41)
![SDLC](https://img.shields.io/badge/SDLC_%7C_STLC-0d0d0d?style=for-the-badge&logoColor=00ff41)
![Agile](https://img.shields.io/badge/Agile_%7C_Scrum-0d0d0d?style=for-the-badge&logoColor=00ff41)

### 💻 Languages

![JavaScript](https://img.shields.io/badge/JavaScript-0d0d0d?style=for-the-badge&logo=javascript&logoColor=00ff41)
![Python](https://img.shields.io/badge/Python-0d0d0d?style=for-the-badge&logo=python&logoColor=00ff41)
![Java](https://img.shields.io/badge/Java-0d0d0d?style=for-the-badge&logo=openjdk&logoColor=00ff41)
![C](https://img.shields.io/badge/C-0d0d0d?style=for-the-badge&logo=c&logoColor=00ff41)
![C++](https://img.shields.io/badge/C++-0d0d0d?style=for-the-badge&logo=cplusplus&logoColor=00ff41)
![Bash](https://img.shields.io/badge/Bash-0d0d0d?style=for-the-badge&logo=gnubash&logoColor=00ff41)

### 🌐 Frontend

![React](https://img.shields.io/badge/React.js-0d0d0d?style=for-the-badge&logo=react&logoColor=00ff41)
![HTML5](https://img.shields.io/badge/HTML5-0d0d0d?style=for-the-badge&logo=html5&logoColor=00ff41)
![CSS3](https://img.shields.io/badge/CSS3-0d0d0d?style=for-the-badge&logo=css3&logoColor=00ff41)

### 🗄️ Backend & Databases

![Node.js](https://img.shields.io/badge/Node.js-0d0d0d?style=for-the-badge&logo=node.js&logoColor=00ff41)
![Express.js](https://img.shields.io/badge/Express.js-0d0d0d?style=for-the-badge&logo=express&logoColor=00ff41)
![MongoDB](https://img.shields.io/badge/MongoDB-0d0d0d?style=for-the-badge&logo=mongodb&logoColor=00ff41)
![MySQL](https://img.shields.io/badge/MySQL-0d0d0d?style=for-the-badge&logo=mysql&logoColor=00ff41)
![JWT](https://img.shields.io/badge/JWT_Auth-0d0d0d?style=for-the-badge&logo=jsonwebtokens&logoColor=00ff41)
![REST API](https://img.shields.io/badge/REST_API-0d0d0d?style=for-the-badge&logo=postman&logoColor=00ff41)

### 🛠️ Tools & Platforms

![Git](https://img.shields.io/badge/Git-0d0d0d?style=for-the-badge&logo=git&logoColor=00ff41)
![VS Code](https://img.shields.io/badge/VS_Code-0d0d0d?style=for-the-badge&logo=visual-studio-code&logoColor=00ff41)
![Postman](https://img.shields.io/badge/Postman-0d0d0d?style=for-the-badge&logo=postman&logoColor=00ff41)
![Vercel](https://img.shields.io/badge/Vercel-0d0d0d?style=for-the-badge&logo=vercel&logoColor=00ff41)
![Render](https://img.shields.io/badge/Render-0d0d0d?style=for-the-badge&logo=render&logoColor=00ff41)
![Brevo](https://img.shields.io/badge/Brevo_SMTP_API-0d0d0d?style=for-the-badge&logoColor=00ff41)

---

## `> ls ./projects`

<div align="center">

### 💈 SalonQ — Salon Appointment & Queue Management

</div>

```yaml
Project  : SalonQ
Type     : Full-Stack MERN · Production App
Year     : 2025
Status   : 🟢 LIVE
Deploy   : Frontend → Vercel  |  Backend → Render  |  DB → MongoDB Atlas
CI/CD    : GitHub Actions — auto deploy on every push
```

<div align="center">

[![🌐 LIVE DEMO](https://img.shields.io/badge/🌐_LIVE_DEMO-VISIT_NOW-00ff41?style=for-the-badge&logoColor=black)](https://salon-q-eight.vercel.app)

</div>

| Feature | Detail |
|:--------|:-------|
| 🔐 Auth | OTP authentication + JWT-secured sessions with role-based access (Customer & Admin) |
| 📊 Dashboard | Real-time Admin dashboard for live queue tracking, booking approvals & slot management |
| 📡 API | RESTful APIs for booking, approval workflows, queue & user management |
| 📧 Email Fix | Migrated from SMTP (port 465/587) to Brevo HTTP API (port 443) — resolved Render port-blocking, restored OTP delivery with zero frontend changes |
| ⚙️ CI/CD | GitHub Actions pipeline — automated build & deploy on every push |
| 🗃️ DB | MongoDB Atlas with Mongoose schemas, CRUD, indexing |

**Stack:**
![React](https://img.shields.io/badge/React.js-0d0d0d?style=flat-square&logo=react&logoColor=00ff41)
![Node.js](https://img.shields.io/badge/Node.js-0d0d0d?style=flat-square&logo=node.js&logoColor=00ff41)
![Express](https://img.shields.io/badge/Express.js-0d0d0d?style=flat-square&logo=express&logoColor=00ff41)
![MongoDB](https://img.shields.io/badge/MongoDB-0d0d0d?style=flat-square&logo=mongodb&logoColor=00ff41)
![JWT](https://img.shields.io/badge/JWT-0d0d0d?style=flat-square&logo=jsonwebtokens&logoColor=00ff41)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-0d0d0d?style=flat-square&logo=github-actions&logoColor=00ff41)
![Vercel](https://img.shields.io/badge/Vercel-0d0d0d?style=flat-square&logo=vercel&logoColor=00ff41)
![Render](https://img.shields.io/badge/Render-0d0d0d?style=flat-square&logo=render&logoColor=00ff41)
![Brevo](https://img.shields.io/badge/Brevo_API-0d0d0d?style=flat-square&logoColor=00ff41)

---

<div align="center">

### 🎓 MCA Portal — Secure College Management System

</div>

```yaml
Project  : MCA Portal
Type     : Full-Stack MERN · Docker · Cloud Deployment
Year     : 2026
Status   : 🟢 LIVE ON AWS EC2
Deploy   : Docker Compose + AWS EC2  |  Automated via GitHub Actions CI/CD
```

<div align="center">

[![🌐 LIVE APP](https://img.shields.io/badge/🌐_LIVE_APP-AWS_EC2-00ff41?style=for-the-badge&logoColor=black)](http://51.20.31.138:5173)
[![⚙️ GitHub](https://img.shields.io/badge/⚙️_SOURCE_CODE-GITHUB-0d0d0d?style=for-the-badge&logo=github&logoColor=00ff41)](https://github.com/ChinthanRai)

</div>

| Feature | Detail |
|:--------|:-------|
| 👥 Roles | Admin, Faculty & Student — isolated access, dashboards & permissions per role |
| 🔑 Auth | OTP-based student onboarding with 7-step approval workflow (`test_flow.js`) |
| 🧪 API Testing | 55+ REST endpoints validated via custom Node.js integration scripts |
| ⚡ Performance | ~220ms avg API response time confirmed across all routes |
| 🐳 Containerised | Full stack dockerised with Docker Compose; `.env` secrets management |
| ☁️ Cloud | Deployed on AWS EC2; SSH access, IAM, VPC security groups configured |
| ⚙️ CI/CD | GitHub Actions — automated build & deploy triggered on every commit |

**Stack:**
![React](https://img.shields.io/badge/React.js-0d0d0d?style=flat-square&logo=react&logoColor=00ff41)
![Node.js](https://img.shields.io/badge/Node.js-0d0d0d?style=flat-square&logo=node.js&logoColor=00ff41)
![Express](https://img.shields.io/badge/Express.js-0d0d0d?style=flat-square&logo=express&logoColor=00ff41)
![MongoDB](https://img.shields.io/badge/MongoDB-0d0d0d?style=flat-square&logo=mongodb&logoColor=00ff41)
![Docker](https://img.shields.io/badge/Docker_Compose-0d0d0d?style=flat-square&logo=docker&logoColor=00ff41)
![AWS EC2](https://img.shields.io/badge/AWS_EC2-0d0d0d?style=flat-square&logo=amazon-aws&logoColor=00ff41)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-0d0d0d?style=flat-square&logo=github-actions&logoColor=00ff41)
![Shell](https://img.shields.io/badge/Shell_Scripts-0d0d0d?style=flat-square&logo=gnubash&logoColor=00ff41)

---

## `> cat internship.log`

<div align="center">

```
┌─────────────────────────────────────────────────────────────────────┐
│                                                                     │
│   🏢  DevOps Intern @ Rooman Technologies                          │
│   📅  Feb 2026 – May 2026  ·  Remote, Bengaluru, Karnataka         │
│   🏆  Score: 100 / 100  ·  Cloud Computing & DevOps Certified      │
│                                                                     │
│   ▸ Provisioned AWS EC2 — SSH, IAM roles, VPC, S3                   │
│   ▸ Built GitHub Actions CI/CD pipelines (build → test → deploy)    │
│   ▸ Containerised multi-service apps with Docker & Docker Compose   │
│   ▸ Wrote Bash automation scripts for routine ops tasks             │
│   ▸ Monitored live server logs; traced & resolved failures          │
│   ▸ Ran smoke & sanity checks post-deploy for env validation        │
│   ▸ Applied STLC principles; documented post-release checklists     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

</div>

---

## `> cat certs_and_wins.log`

<div align="center">

| 🎖️ Certification | Issuer | Score |
|:----------------|:-------|:-----:|
| Cloud Computing & DevOps | Rooman Technologies | **100 / 100** 🔥 |
| Machine Learning | IBM | ✅ |
| Deep Learning (TensorFlow) | Coursera | ✅ |
| Zscaler Zero Trust Associate | Zscaler | ✅ |
| UiPath RPA Developer | UiPath | ✅ |
| Advanced Python | Ethnotech Academy | ✅ |
| HTML5 | Ethnotech Academy | ✅ |

<br/>

| 🏅 Achievement | Event | Position |
|:--------------|:------|:--------:|
| Mathematics | Govinda Dasa College | 🥇 1st Place |
| Physics Hunt | Mangalore University | 🥇 1st Place |
| Mathematics | Canara College | 🥈 2nd Place ×2 |

</div>

---

## `> cat activity_graph.log`

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=ChinthanRai&theme=react-dark&bg_color=0d0d0d&color=00ff41&line=00ff41&point=ffffff&area=true&hide_border=true" width="100%"/>
</div>

---

## `> top-langs --compact`

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ChinthanRai&layout=compact&theme=chartreuse-dark&hide_border=true&bg_color=0d0d0d&title_color=00ff41&text_color=ffffff" />
  <br/><br/>
  <img src="https://komarev.com/ghpvc/?username=ChinthanRai&label=PROFILE+VIEWS&color=00ff41&style=for-the-badge&labelColor=0d0d0d" />
</div>

---

## `> ping --connect`

<div align="center">

[![Gmail](https://img.shields.io/badge/GMAIL-chinthanrai.123%40gmail.com-00ff41?style=for-the-badge&logo=gmail&logoColor=black&labelColor=0d0d0d)](mailto:chinthanrai.123@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LINKEDIN-chinthan--rai--kukkuvalli-00ff41?style=for-the-badge&logo=linkedin&logoColor=black&labelColor=0d0d0d)](https://www.linkedin.com/in/chinthan-rai-kukkuvalli/)
[![GitHub](https://img.shields.io/badge/GITHUB-ChinthanRai-00ff41?style=for-the-badge&logo=github&logoColor=black&labelColor=0d0d0d)](https://github.com/ChinthanRai)

</div>

<br/>

<div align="center">

```
╔═══════════════════════════════════════════════════════════════╗
║                                                               ║
║   > Available for: Full-Stack · DevOps · QA Engineering       ║
║   > Skills: MERN · Docker · AWS · Selenium · Playwright       ║
║   > Delivered: 2 production apps · 55+ endpoints validated    ║
║   > Internship: Rooman Technologies · Score 100/100 🔥        ║
║                                                               ║
║               [ SYSTEM READY. AWAITING INPUT. ]               ║
║                                                               ║
╚═══════════════════════════════════════════════════════════════╝
```

</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0d0d,50:00ff41,100:0d0d0d&height=120&section=footer" width="100%"/>
</div>
